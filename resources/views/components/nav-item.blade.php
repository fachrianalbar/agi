...
<button class="sidebar-nav-item @if($active) active @endif"
        data-page="{{ $route }}"
        onclick="window.appNavigate('{{ $route }}')">
  <span class="sidebar-nav-icon">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">{!! $icons[$icon] ?? $icons['dashboard'] !!}</svg>
  </span>
